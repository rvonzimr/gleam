...
// build/dev/javascript/shared/groceries.mjs
var GroceryItem = class extends CustomType {
...
function grocery_item_to_json(grocery_item) {
  ...
  return object2(
    toList([["name", string3(name)], ["quantity", int3(quantity)]])
  );
}
...
var UserAddedItem = class extends CustomType {
};
var UserTypedNewItem = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function view_new_item(new_item) {
  return div(
    ...
    toList([
      ...
      button(
        toList([on_click(new UserAddedItem())]),
        toList([text3("Add")])
      )
    ])
  );
...
function view_grocery_list(items) {
  if (items instanceof Empty) {
    return p(toList([]), toList([text3("No groceries")]));
  } else {
    return ul(
      toList([]),
      index_map(
...
function view(model) {
  return div(
    toList([class$("bg-primary-01 flex flex-col gap-4")]),
    toList([
      ...
      (() => {
        let $ = model.error;
        if ($ instanceof Some) {
          return div(toList([]), toList([text3("Oopise.")]));
        } else {
          return none2();
        }
      })()
...
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 407, end: 455, pattern_start: 418, pattern_end: 423 }
    );
  }
  return void 0;
}
